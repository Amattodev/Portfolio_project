...
# デプロイ
Vercel：https://vercel.com/amattodevs-projects

## フロントエンドのデプロイ
・環境変数のチェック（本番環境用のものか）
    - 特に連携するバックエンドのURLが本番環境用かチェック
・Vercelコマンドでデプロイ
・環境変数を設定する

## バックエンドのデプロイ
・環境変数のチェック
・MongoDBのURLをローカルのものから本番環境用へ
    - Mongo Atlasで本番環境用のURIを生成
    - 本番環境用の DBを作成
・フロントエンドのデプロイURLに更新
    - プレビュー用と本番環境用を間違えないように
・環境変数の設定
・デプロイ
    - エラー出たらLogかFunctionをチェック
    - 多分データベース関係

## フロントエンドの再デプロイ
・環境変数のバックエンドのURL部分を更新
・デプロイ
・CORSエラー：異なるオリジンからリソースを共有することを許可されていない？
    - 環境変数のチェック（特にURLの部分）
    - リクエストヘッダーをチェック（開発者ツール）
・APIエラー
    - 開発者ツールのNetworkタブチェック
